#!/usr/bin/env python
# -*- coding: utf-8 -*-
#Created by Chukwunyere Igbokwe on January 12, 2017 by 9:24 PM
...
right_line_hood_range = [int(mean_lane_middle + lane_width / 2 - hood_range_perc / 2 * capture_width), \
                         int(mean_lane_middle + lane_width / 2 + hood_range_perc / 2 * capture_width)]

left_line_hood_range = [int(mean_lane_middle - lane_width / 2 - hood_range_perc / 2 * capture_width), \
                        int(mean_lane_middle - lane_width / 2 + hood_range_perc / 2 * capture_width)]
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    if lines_matl.size > 1:
        ...
        b = np.subtract(lines_matl[:, 1] + upper_frame_row, np.multiply(slope, (lines_matl[:, 0] + left_line_hood_capture_range[0])))
    ...
    if lines_matr.size > 1:
        ...
        b = np.subtract(lines_matr[:, 1] + upper_frame_row, np.multiply(slope, \
                                                                        (lines_matr[:, 0] +
                                                                         right_line_hood_capture_range[0] - add_width)))
        hood_col = np.divide(np.subtract(upper_frame_row + height, b), slope)
        horizon_col = np.divide(np.subtract(horizon_row, b), slope)
    ...
    cv2.line(img, (int(left_line_hood_range[0]), upper_frame_row), (int(left_line_hood_range[0]), hood_row),
             (255, 0, 0), 2)
    cv2.line(img, (int(left_line_hood_range[1] + add_width), upper_frame_row),
             (int(left_line_hood_range[1] + add_width), hood_row), (255, 0, 0), 2)
    cv2.line(img, (int(left_line_hood_range[1]), hood_row - 10), (int(left_line_hood_range[1]), hood_row), (255, 0, 0),
             2)
    cv2.line(img, (int(right_line_hood_range[0]), hood_row - 10), (int(right_line_hood_range[0]), hood_row),
             (255, 0, 0), 2)
    cv2.line(img, (int(right_line_hood_range[0] - add_width), upper_frame_row),
             (int(right_line_hood_range[0] - add_width), hood_row), (255, 0, 0), 2)
    cv2.line(img, (int(right_line_hood_range[1]), upper_frame_row), (int(right_line_hood_range[1]), hood_row),
             (255, 0, 0), 2)
    ...
    cv2.line(img, (mean_lane_middle - lane_width / 2, 0), (mean_lane_middle - lane_width / 2, capture_height),
             (0, 0, 255), 1)
    cv2.line(img, (mean_lane_middle + lane_width / 2, 0), (mean_lane_middle + lane_width / 2, capture_height),
             (0, 0, 255), 1)
